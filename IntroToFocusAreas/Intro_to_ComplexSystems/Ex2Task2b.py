...
for i in range(100+1):
    ...
    I_counts.append(list(counts[-1])[-1])


fig, ax = plt.subplots()
ax.hist(I_counts, bins = 30)
average = avg = str(round(statistics.mean(I_counts),2))
textstr = "Average number\nof infected individuals\nat the end time: {avg}".format(avg = average)
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
ax.text(0.6, 0.95, textstr, transform=ax.transAxes, fontsize=10,
        verticalalignment='top', bbox=props)
plt.grid()
plt.title("number of infected individuals at time t = 100 for 100 simulations")
plt.xlabel("number of infected at time t = 100")
